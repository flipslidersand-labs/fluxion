...
mod tests {
    ...
    #[test]
    fn terminal_states() {
        assert!(JobStatus::Succeeded { elapsed: Duration::ZERO }.is_terminal());
        assert!(JobStatus::Failed { elapsed: Duration::ZERO, reason: "err".into() }.is_terminal());
    ...
    fn labels_match_expected_strings() {
        ...
        assert_eq!(JobStatus::Succeeded { elapsed: Duration::ZERO }.label(), "SUCCESS");
        assert_eq!(
            JobStatus::Failed { elapsed: Duration::ZERO, reason: String::new() }.label(),
            "FAILED"
        );
